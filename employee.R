...
setwd("D:/Data Science edwisor/Projects/Employee Absentism")
getwd()
...
catagorical_vars = colnames(df)[!colnames(df) %in% continuous_vars]

#-----------------------------------Exploratory Data analysis-------------------------------------------
## Univariate Exploratory Analysis ##

## here am plotting histograms for all continous variables and assigning them to variables to plot later
j=0
for (i in 1:length(continuous_vars)){
  
  assign(paste0('gg',j),ggplot(data = df,aes_string(x=(continuous_vars[i])))+geom_histogram(bins=50)
         +xlab(continuous_vars[i])+theme_bw())
  j=j+1
}

## plotting using plot_grid in a single page
plot_grid(gg1,gg2,gg3,gg4)


## plotting bar plots for categorical variables

## grouping target variable with each categorical variable and plotting bar plots 
j=0
for (i in categorical_vars){
  assign(paste0('gc',j),df %>% group_by(x=df[,i])%>%
           summarise('sum'=mean(Absenteeism.time.in.hours,na.rm = T))%>%
           ggplot(aes(x=x,y=sum))+geom_bar(stat = 'identity',fill='blue')+xlab(i)+theme_bw())
  j=j+1
}

## plotting using plot_grid in a single page
plot_grid(gc5,gc6,gc7,gc8)

## Bivariate Exploratory Analysis

## plotting scatter plots for all continuous variables w.r.t target variable
j=0
for (i in continuous_vars){
  
  assign(paste0('gb',j),ggplot(data = df,aes_string(x=i,y='Absenteeism.time.in.hours'))
         +geom_point(na.rm = T)+xlab(i)+ylab('Absenteeism.time.in.hours'))
  
  j=j+1
}

## plotting using plot_grid in a single page
plot_grid(gb1,gb2,gb3,gb4)

## plotting histograms for all continuous varibles using psych package
install.packages('psych')
library(psych)
multi.hist(df[,continuous_vars], dcol = c("blue", "red"),
